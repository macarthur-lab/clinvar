...
#sort
job.add("(cat IN:clinvar_table_normalized.tsv | head -1 > OUT:clinvar_record.tsv ) && " + # header row
    "(cat IN:clinvar_table_normalized.tsv | tail -n +2 | egrep -v \"^[XYM]\" | sort -k1,1n -k2,2n -k3,3 -k4,4 >> OUT:clinvar_record.tsv ) && " + # numerically sort chroms 1-22
    "(cat IN:clinvar_table_normalized.tsv | tail -n +2 | egrep \"^[XYM]\" | sort -k1,1 -k2,2n -k3,3 -k4,4 >> OUT:clinvar_record.tsv )")     # lexicogaraphically sort non-numerical chroms at end

job.add("bgzip -c IN:clinvar_record.tsv > OUT:clinvar_record.tsv.gz")
job.add("tabix -S 1 -s 1 -b 2 -e 2 IN:clinvar_record.tsv.gz", output_filenames=["clinvar_record.tsv.gz.tbi"])
job.add("cp IN:clinvar_record.tsv.gz IN:clinvar_record.tsv.gz.tbi ../", output_filenames=["../clinvar_record.tsv", "../clinvar_record.tsv.gz", "../clinvar_record.tsv.gz.tbi"])

# join information from the tab-delimited summary to the normalized genomic coordinates
job.add("Rscript IN:join_data.R IN:%s" % variant_summary_table, input_filenames=['clinvar_record.tsv'], output_filenames=['clinvar_combined.tsv'])
...
job.add("python -u IN:dedup_clinvar.py -i IN:clinvar_sorted.tsv -o OUT:clinvar.tsv")

job.add("bgzip -c IN:clinvar.tsv > OUT:clinvar.tsv.gz")
